...
struct MeetingsNavigationTests {
    ...
    @Test("live transcript checkpoints persist after another meeting owns the UI")
    func liveTranscriptCheckpointsPersistWithoutUIOwnership() throws {
        let store = try makeStore()
        let id = try store.createLiveMeeting(title: "Background Capture", calendarEventID: nil, startTime: Date())
        let controller = makeController(dictationStore: store)
        let entry = LiveTranscriptCheckpointEntry(
            timestampLabel: "11:45:02",
            speaker: "Others",
            startSeconds: 2,
            endSeconds: 3,
            text: "Persist this queued final chunk."
        )
        let generation = controller.advanceLiveMeetingCheckpointGeneration(meetingID: id)

        let updatedUI = controller.deliverLiveMeetingTranscriptCheckpointEntries(
            [entry],
            meetingID: id,
        ...
        #expect(!updatedUI)
        #expect(controller.appState.liveMeetingTranscript.isEmpty)
        #expect(
            try store.liveTranscriptCheckpointText(meetingID: id)
                == "[11:45:02] Others: Persist this queued final chunk."
        )
    }

    @Test("resuming the same row rejects old-generation checkpoints")
    func resumedMeetingRejectsCheckpointABAFromCompletedCapture() throws {
        let store = try makeStore()
        let startedAt = Date()
        let id = try store.createLiveMeeting(
            title: "Resume checkpoint generation",
            calendarEventID: nil,
            startTime: startedAt
        )
        let controller = makeController(dictationStore: store)
        let oldGeneration = controller.advanceLiveMeetingCheckpointGeneration(meetingID: id)
        ...
        try store.completeLiveMeeting(
            ...
            rawTranscript: "First capture",
            formattedNotes: "First capture",
            micAudioPath: nil,
            systemAudioPath: nil
        )

        // Production advances ownership before prepareMeetingForResume opens
        // the completed row again. Preserve that exact ordering here.
        let resumedGeneration = controller.advanceLiveMeetingCheckpointGeneration(meetingID: id)
        _ = try store.prepareMeetingForResume(id: id)

        let staleEntry = LiveTranscriptCheckpointEntry(
            timestampLabel: "11:45:02",
            speaker: "Others",
            startSeconds: 2,
            endSeconds: 3,
            text: "Stale first capture"
        )
        let currentEntry = LiveTranscriptCheckpointEntry(
            timestampLabel: "11:45:04",
            speaker: "You",
            startSeconds: 4,
            endSeconds: 5,
            text: "Current resumed capture"
        )

        let staleUpdatedUI = controller.deliverLiveMeetingTranscriptCheckpointEntries(
            [staleEntry],
            meetingID: id,
            generation: oldGeneration
        )
        #expect(!staleUpdatedUI)
        #expect(try store.liveTranscriptCheckpointText(meetingID: id) == nil)

        let currentUpdatedUI = controller.deliverLiveMeetingTranscriptCheckpointEntries(
            [currentEntry],
            meetingID: id,
            generation: resumedGeneration
        )
        #expect(!currentUpdatedUI)
        #expect(
            try store.liveTranscriptCheckpointText(meetingID: id)
                == "[11:45:04] You: Current resumed capture"
        )
    }
